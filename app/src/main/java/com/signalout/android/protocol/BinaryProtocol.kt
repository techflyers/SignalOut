package com.signalout.android.protocol
...
enum class MessageType(val value: UByte) {
    ...
    CALL_OFFER(0x30u), // SDP offer
    CALL_ANSWER(0x31u), // SDP answer
...
data class SignaloutPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = SignaloutPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.signalout.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): SignaloutPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: SignaloutPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): SignaloutPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): SignaloutPacket? {
        try {
