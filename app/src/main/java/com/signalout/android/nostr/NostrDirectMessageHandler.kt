package com.signalout.android.nostr
...
import com.signalout.android.model.PrivateMessagePacket
import com.signalout.android.protocol.SignaloutPacket
import com.signalout.android.services.SeenMessageStore
import com.signalout.android.ui.ChatState
import com.signalout.android.ui.MeshDelegateHandler
import com.signalout.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.signalout.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("signalout1:")) return@launch

                val base64Content = content.removePrefix("signalout1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = SignaloutPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.signalout.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.signalout.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = SignaloutFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.signalout.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = SignaloutMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
