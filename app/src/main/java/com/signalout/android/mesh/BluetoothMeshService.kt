package com.signalout.android.mesh

import android.content.Context
import android.util.Log
import com.signalout.android.crypto.EncryptionService
import com.signalout.android.model.SignaloutMessage
import com.signalout.android.model.RoutedPacket
import com.signalout.android.model.IdentityAnnouncement
import com.signalout.android.model.NoisePayload
import com.signalout.android.model.NoisePayloadType
import com.signalout.android.protocol.SignaloutPacket
import com.signalout.android.protocol.MessageType
import com.signalout.android.protocol.SpecialRecipients
import com.signalout.android.model.RequestSyncPacket
import com.signalout.android.sync.GossipSyncManager
import com.signalout.android.util.toHexString
import com.signalout.android.services.VerificationService
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { com.signalout.android.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    
    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.signalout.android.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.signalout.android.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.signalout.android.util.NotificationIntervalManager()
    )
    ...
    init {
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.signalout.android.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.signalout.android.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.signalout.android.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
        ...
        gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
            override fun sendPacket(packet: SignaloutPacket) {
                connectionManager.broadcastPacket(RoutedPacket(packet))
            }
            override fun sendPacketToPeer(peerID: String, packet: SignaloutPacket) {
                connectionManager.sendPacketToPeer(peerID, packet)
            }
            override fun signPacketForBroadcast(packet: SignaloutPacket): SignaloutPacket {
        ...
        // Initialize CallManager bindings
        try {
            val callManager = com.signalout.android.call.CallManager.getInstance(context)
            callManager.onSendSignaling = { toPeerID, payload ->
                sendCallSignaling(toPeerID, payload)
            }
            Log.d(TAG, "CallManager signaling bound to MeshService")
        } catch (e: Exception) {
            Log.e(TAG, "Error binding CallManager", e)
        }
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.signalout.android.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.signalout.android.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            ...
            override fun sendHandshakeResponse(peerID: String, response: ByteArray) {
                ...
                val responsePacket = SignaloutPacket(
                    version = 1u,
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun verifySignature(packet: SignaloutPacket, peerID: String): Boolean {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.signalout.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.signalout.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
            ...
            // Callbacks
            override fun onMessageReceived(message: SignaloutMessage) {
                // Always reflect into process-wide store so UI can hydrate after recreation
                try {
                    when {
                        message.isPrivate -> {
                            val peer = message.senderPeerID ?: ""
                            if (peer.isNotEmpty()) com.signalout.android.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.signalout.android.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.signalout.android.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.signalout.android.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleCallSignaling(routed: RoutedPacket) {
                try {
                    val peerID = routed.peerID ?: return
                    val callManager = com.signalout.android.call.CallManager.getInstance(context)
                    callManager.handleSignalingMessage(peerID, routed.packet.payload)
                } catch (e: Exception) {
                    Log.e(TAG, "Error handling call signaling", e)
                }
            }

            override fun validatePacketSecurity(packet: SignaloutPacket, peerID: String): Boolean {
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.signalout.android.util.AppConstants.MESSAGE_TTL_HOPS
            ...
            override fun handleFragment(packet: SignaloutPacket): SignaloutPacket? {
        ...
        override fun onPacketReceived(packet: SignaloutPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            // Log incoming for debug graphs (do not double-count anywhere else)
            try {
                com.signalout.android.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.signalout.android.ui.debug.DebugSettingsManager.getInstance()
    ...
    /**
     * Send Call Signaling via mesh. Uses CALL_OFFER as generic routing indicator for mesh.
     * Actual subtype (offer, answer, ice, etc.) is encoded inside payload by CallSignaling.kt.
     */
    private fun sendCallSignaling(recipientPeerID: String, payload: ByteArray) {
        try {
            Log.d(TAG, "📤 sendCallSignaling to: $recipientPeerID (${payload.size} bytes)")
            serviceScope.launch {
                val packet = SignaloutPacket(
                    version = 1u,
                    type = MessageType.CALL_OFFER.value,
                    senderID = hexStringToByteArray(myPeerID),
     ...
     */
    fun sendFileBroadcast(file: com.signalout.android.model.SignaloutFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.signalout.android.model.SignaloutFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.signalout.android.model.NoisePayload(
                            type = com.signalout.android.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.signalout.android.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.signalout.android.model.NoisePayload(
                        type = com.signalout.android.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.signalout.android.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.signalout.android.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.signalout.android.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.signalout.android.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.signalout.android.model.NoisePayload(
                    type = com.signalout.android.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = SignaloutPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.signalout.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.signalout.android.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.signalout.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    com.signalout.android.services.meshgraph.MeshGraphService.getInstance()
                        .updateFromAnnouncement(myPeerID, nickname, directPeers, System.currentTimeMillis().toULong())
                } catch (_: Exception) { }
            } catch (_: Exception) { }
            
            val announcePacket = SignaloutPacket(
     ...
     */
    fun getSessionState(peerID: String): com.signalout.android.noise.NoiseSession.NoiseSessionState {
     ...
     */
    private fun signPacketBeforeBroadcast(packet: SignaloutPacket): SignaloutPacket {
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.signalout.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
...
interface BluetoothMeshDelegate {
    fun didReceiveMessage(message: SignaloutMessage)
