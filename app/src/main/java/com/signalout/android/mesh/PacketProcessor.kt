...
class PacketProcessor(private val myPeerID: String) {
     ...
     */
    private suspend fun handleCallSignaling(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing Call Signaling from ${formatPeerForLog(peerID)}")
        delegate?.handleCallSignaling(routed)
    }
...
interface PacketProcessorDelegate {
    // Security validation
    fun validatePacketSecurity(packet: SignaloutPacket, peerID: String): Boolean
